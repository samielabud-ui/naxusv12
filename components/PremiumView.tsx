...
const PremiumView: React.FC<PremiumViewProps> = ({ userStats }) => {
  ...
  // Progresso agora vem direto de userStats (Firebase via App.tsx)
  ...
  return (
    <div className="animate-in fade-in duration-700 pb-20 w-full">
      ...
      <div className="max-w-[1800px] mx-auto px-4 md:px-8">
        {/* Continuar Assistindo - Usando Firebase */}
        {!selectedPlatform && lastWatched && (
          <section className="mb-16 animate-in fade-in slide-in-from-left-4 duration-500">
            ...
            <div 
              onClick={() => { 
                setSelectedPlatform(lastWatched.platformId); 
                setSelectedCourse(lastWatched.courseName); 
                setActiveVideo({ id: lastWatched.lessonId, title: lastWatched.lessonTitle });
              }}
              className="group relative max-w-sm aspect-video bg-neutral-900 rounded-3xl overflow-hidden border border-neutral-800 hover:border-[#00BFA6] transition-all cursor-pointer shadow-2xl"
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-12 h-12 bg-white/10 backdrop-blur rounded-full flex items-center justify-center text-white border border-white/20 group-hover:bg-[#00BFA6] transition-all">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <span className="text-[8px] font-black text-[#00BFA6] uppercase tracking-widest mb-1 block">{lastWatched.courseName}</span>
        ...
        <header className="mb-10">
          <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-4">
            ...
            {selectedPlatform && (
              <>
                <span>/</span>
                <button onClick={() => { setSelectedCourse(null); setActiveVideo(null); }} className={`hover:text-white ${selectedPlatform === 'sanarflix' ? 'text-[#00BFA6]' : 'text-neutral-300'}`}>{PREMIUM_PLATFORMS.find(p => p.id === selectedPlatform)?.title}</button>
              </>
            )}
            {selectedCourse && (
              <>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="relative w-full md:w-96 group">
              <input 
                ...
                className={`w-full bg-neutral-900 border border-neutral-800 rounded-xl py-4 px-12 text-xs text-white focus:border-[#00BFA6] focus:ring-1 focus:ring-[#00BFA6]/30 outline-none transition-all placeholder:text-neutral-700 shadow-2xl`}
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
              <svg 
                className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${search ? 'text-[#00BFA6]' : 'text-neutral-600'}`} 
        ...
        {!selectedPlatform && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 animate-in fade-in duration-500">
            {filteredPlatforms.map((p) => (
              <div key={p.id} onClick={() => setSelectedPlatform(p.id)} className="group relative aspect-video rounded-3xl overflow-hidden bg-neutral-900 border border-neutral-800 hover:border-[#00BFA6] transition-all cursor-pointer shadow-2xl active:scale-95">
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end">
                   <h3 className="text-2xl font-black text-white tracking-tight mb-2 group-hover:text-[#00BFA6] transition-colors">{p.title}</h3>
                   <p className="text-neutral-400 text-xs font-light leading-relaxed mb-4">{p.description}</p>
                   <span className="text-[10px] font-black uppercase tracking-widest text-[#00BFA6] group-hover:translate-x-2 transition-transform inline-block">Acessar →</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {filteredCourses.map((course, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`group relative aspect-[2/3] md:aspect-video rounded-2xl overflow-hidden bg-gradient-to-br from-neutral-900 to-[#0c0c0c] border border-neutral-800 hover:border-white/20 transition-all cursor-pointer shadow-xl hover:shadow-[0_0_30px_rgba(0,191,166,0.15)] hover:-translate-y-1`}
                >
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                    <div className="w-10 h-10 bg-[#00BFA6]/10 backdrop-blur-md rounded-full flex items-center justify-center text-[#00BFA6] border border-[#00BFA6]/30 scale-75 group-hover:scale-100 transition-transform">
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12 animate-in fade-in duration-500">
              ...
              <div className="sticky top-24">
                {activeVideo ? (
                  ...
                  <div className="w-full aspect-video bg-neutral-900/40 rounded-[2rem] flex flex-col items-center justify-center border-2 border-dashed border-neutral-800 text-center p-8">
                    <div className="w-16 h-16 bg-[#00BFA6]/5 rounded-full flex items-center justify-center text-[#00BFA6]/30 mb-6 border border-[#00BFA6]/10"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg></div>
                ...
                {activeVideo && (
                  <div className="mt-8 flex justify-between items-center p-8 bg-neutral-900/40 rounded-[2rem] border border-neutral-800">
                    <div><span className="text-[10px] font-black text-[#00BFA6] uppercase tracking-widest mb-1 block">Aula Atual</span><h3 className="text-2xl font-black text-white tracking-tight">{activeVideo.title}</h3></div>
                    <button onClick={() => markAsWatched(activeVideo.id)} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${watchedVideos.includes(activeVideo.id) ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-600/30' : 'bg-[#00BFA6] text-black hover:bg-emerald-500 hover:text-white'}`}>
            ...
            <div className="w-full lg:w-1/3 max-h-[80vh] overflow-y-auto no-scrollbar space-y-3">
              ...
              {filteredLessons.map((lesson, idx) => (
                <div key={lesson.id} onClick={() => handleLessonSelect(lesson)} className={`group p-4 rounded-2xl border transition-all cursor-pointer flex items-center gap-4 ${activeVideo?.id === lesson.id ? 'bg-[#00BFA6]/10 border-[#00BFA6]/50' : 'bg-neutral-900/30 border-neutral-800 hover:border-neutral-700'}`}>
                  <div className="relative w-24 aspect-video rounded-lg overflow-hidden shrink-0 bg-neutral-800">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center"><svg className={`w-6 h-6 ${activeVideo?.id === lesson.id ? 'text-[#00BFA6]' : 'text-white/40'}`} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg></div>
                  </div>
                  <div className="flex-grow min-w-0">
                    <h4 className={`text-xs md:text-sm font-bold line-clamp-2 ${activeVideo?.id === lesson.id ? 'text-[#00BFA6]' : 'text-neutral-300'}`}>{idx + 1}. {lesson.title}</h4>
                  </div>
                </div>
              ))}
            </div>
        ...
        {selectedCourse && selectedCourse !== 'Embriologia' && (
          <div className="py-20 text-center border-2 border-dashed border-neutral-800 rounded-[3rem] animate-in fade-in duration-500">
             <div className="w-16 h-16 bg-neutral-900 rounded-full flex items-center justify-center text-[#00BFA6]/30 mx-auto mb-6"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg></div>
             ...
             <button onClick={() => setSelectedCourse(null)} className="mt-8 text-[#00BFA6] font-bold uppercase tracking-widest text-xs hover:underline">Voltar ao catálogo</button>
          </div>
        )}
